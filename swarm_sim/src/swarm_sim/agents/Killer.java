...
/**
 * @author traian Killer instances are generated from Prey by TargetManage
 *         class, when the number of Prey reached targetKillNumber value. Killer
 *         has only one purpose to reach target as soon as possible. All
 *         kinematic values are transfer from Prey to Killer. There is a limit
 *         on Killer max speed set by preyMaxSpeed parameter.
 */
...
//		Utils.writeToFile(Math.sqrt(Math.pow(velocity.x, 2) + Math.pow(velocity.z, 2)) + "," +
//				Math.abs(velocity.y) + "," +
//				(Math.sqrt(Math.pow(target.velocity.x, 2) + Math.pow(target.velocity.z, 2)) 
